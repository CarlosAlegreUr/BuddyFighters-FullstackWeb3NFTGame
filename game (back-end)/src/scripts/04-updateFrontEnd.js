...
const FRONT_END_CONTRACTS_TESTING_FILE =
  "test/unitest/contractsInfoTesting.json";
...
module.exports = {
  updateFrontEndData: async function (
    ...
    if (process.env.UPDATE_FRONT_END) {
      const currentAddresses = JSON.parse(fs.readFileSync(filePath), "utf-8");
      ...
      if (chaindID in currentAddresses) {
        currentAddresses[chaindID].forEach((contract) => {
          if (contract.contractName === frontEndContractInfo.contractName)
            needToWrite = false;
        });
